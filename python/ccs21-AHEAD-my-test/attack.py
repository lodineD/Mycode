...
# 老的数据集是一共100000个值
data_path_old = './dataset_origin/1dim_normal_skew00-set_10_5-domain6_attribute1-data.txt'
data_path_new = './dataset/1dim_normal_skew00-set_10_5-domain6_attribute1-data.txt'
...
def check_data(data_path):
    ...
    f.close()
    # print(data_old)
    data_old = list(map(int, data_old))
    print(min(data_old))
    print(max(data_old))
    print(data_old.index(min(data_old)))
    list_character = [min(data_old), max(data_old)]
    # print("list_character的max是"+str(list_character[1]))
...
def attack_RIA(person):
    ...
    list_1 = check_data(data_path_old)
    # 组数
    group_num = 4
    # 步长
    long = (list_1[1] - list_1[0]) / group_num
    # print(long)
    # 随机选择组
    ...
    f = open(data_path_new, "a")
    for m in range(person):
        num = int(choice(data_old))
        # print(num)
        left = list_1[0]+(group_index-1)*long
        right = list_1[0]+group_index*long
        while (num<left) or (num>right):
            num = int(choice(data_old))
        print("第" + str(m+1) + "次随机数为" + str(num))
        f.write(str(num) + '\n')
    f.close()
