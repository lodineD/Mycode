...
def user_record_partition(data_path, ahead_tree_height, domain_size):
    ...
    user_histogram = np.zeros((ahead_tree_height, domain_size[0], domain_size[1]), dtype=np.int64)
    for k, item in enumerate(dataset):
        user_sample_id = np.random.randint(0, ahead_tree_height)
        user_histogram[user_sample_id, item[0], item[1]] += 1
...
# record query errors
def ahead_tree_query_error_recorder(ahead_tree, real_frequency, query_interval_table, domain_size, MSEDict, fake_user):
    ...
    for i, query_interval in enumerate(query_interval_table):
        ...
        var_consistency = math.pow(real_frequency_value - estimated_frequency_value, 2)
...
def main_fun(repeat_time, domain_size, branch, ahead_tree_height, theta, real_frequency, query_interval_table, epsilon, data_name, data_size_name, domain_name):
    repeat = 0
    MSEDict = {'rand': []}

    # 在这里加假用户，opg方法, 不用就取-1
    fake_user = -1
    ...
    while repeat < repeat_time:
        ...
        ahead_tree_query_error_recorder(ahead_tree, real_frequency, query_interval_table, domain_size, MSEDict, fake_user=fake_user)
...
if __name__ == "__main__":
    ...
    # load query table
    query_path = './query_table/rand_query_domain8_attribute{}.txt'.format(data_dimension)
    ...
    # select dataset
    data_name = '2dim_laplace01'
    data_size_name = 'set_10_5'
    domain_name = 'domain8_attribute{}'.format(data_dimension)
    ...
    # calculate theta
    theta = theta_calculation(ahead_tree_height, epsilon, data_size, branch, data_dimension)
    
    # running 
    main_fun(repeat_time, domain_size, branch, ahead_tree_height, theta, real_frequency, query_interval_table, epsilon, data_name, data_size_name, domain_name)
